edits, I# Task Plan: Connect Orders API to Orders Service with Full CRUD + Persistence
...
## Authorization flow (get_order)

- [x] `findOne` in order-service now accepts `{ id, securityLevel, userId }` and enforces ownership/privacy
- [x] Order-service controller passes the payload directly (no broken `req.user` reference)
- [x] Api-gateway `findOne` sends `{ id, userId, securityLevel }` from `req.user` (JWT payload)
</content>
